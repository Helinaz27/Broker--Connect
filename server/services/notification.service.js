...
const saveNotification = async ({ userId, type, title, body, path, referenceId }) => {
...
export const notifyKYCApproved = async ({ userId, userEmail, firstName }) => {
  try {
    await saveNotification({
      ...
      type:  'kyc_approved',
      title: 'KYC Approved!',
      body:  'Congratulations! Your identity verification has been approved. You now have full access to all platform features.',
      path:  '/kyc/status',
    });
...
export const notifyKYCRejected = async ({ userId, userEmail, firstName, reviewNote }) => {
  try {
    await saveNotification({
      userId,
      type:  'kyc_rejected',
      ...
      body:  reviewNote
        ? `Your KYC was rejected. Reason: ${reviewNote}. Please re-submit with correct documents.`
...
export const notifyPaymentSuccess = async ({ userId, coinsReceived, amountBirr }) => {
  try {
    await saveNotification({
      userId,
...
export const notifyNewContact = async ({ userId, viewerName, listingTitle, listingId }) => {
  try {
    await saveNotification({
      userId,
      type:        'new_contact',
...
export const notifyPostExpired = async ({ userId, listingTitle, listingId }) => {
  try {
    await saveNotification({
      userId,
      type:        'post_expired',
...
export const notifyInsufficientCoins = async ({ userId, coinsNeeded, coinsAvailable }) => {
  try {
    await saveNotification({
      userId,
...
export const getMyNotificationsService = async ({ userId, page, limit, isRead }) => {
...
export const adminGetAllNotificationsService = async ({ page, limit, userId, type }) => {
  const skip  = (page - 1) * limit;
  const where = {};
