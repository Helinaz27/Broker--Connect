...
export default function CarDetailPage() {
  ...
  const gallery = car.images || [car.image];

  return (
    <main className="flex-1 py-8 md:py-16">
      <div className="container mx-auto px-6 max-w-6xl">
        <Link href="/car-listings" className="inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-muted-foreground hover:text-primary mb-8 transition-colors">
          ...
          <div className="space-y-6">
            <div className="bg-card border border-border/50 rounded-[2.5rem] overflow-hidden shadow-2xl relative group aspect-[16/10]">
              ...
              <div className="absolute top-6 left-6 flex gap-3">
                ...
                <span className={`backdrop-blur-md text-[9px] font-black uppercase tracking-widest px-4 py-2 rounded-full border border-white/10 shadow-xl ${car.type === 'rent' ? 'bg-blue-500/90 text-white' : 'bg-emerald-500/90 text-white'}`}>
                  For {car.type || 'Sale'}
                </span>
              </div>
          ...
          <div className="space-y-8">
            <div className="bg-card/50 backdrop-blur-xl border border-border/50 rounded-[2.5rem] p-8 md:p-10 shadow-xl">
              ...
              <div className="p-6 bg-muted/30 border border-border/40 rounded-3xl mb-10">
                ...
                {car.type === 'rent' && <p className="text-[10px] font-bold text-muted-foreground mt-1 uppercase">Per Day/Week</p>}
              </div>
              ...
              <Button asChild className="w-full h-16 rounded-[1.5rem] bg-primary text-white font-black uppercase tracking-widest text-xs shadow-xl shadow-primary/20 hover:scale-[1.02] transition-all group">
                <a href={`mailto:contact@brokerconnect.et?subject=Asset Inquiry: ${encodeURIComponent(car.title)}`}>
                  Initiate Connection
