  ...
  return (
    <div className="flex flex-col min-h-screen bg-background relative overflow-hidden">
      ...
      <div className="flex-1 flex relative z-10">
        {/* Sidebar */}
        <aside
          className={`${isSidebarCollapsed ? "w-20" : "w-72"} bg-card/50 backdrop-blur-xl border-r border-border/50 transition-all duration-500 flex flex-col sticky top-16 h-[calc(100vh-64px)] overflow-y-auto z-40 group/sidebar`}
        >
          ...
          <nav className="flex-1 p-4 space-y-2">
            ...
            >
              <LayoutDashboard
                className={`h-5 w-5 flex-shrink-0 transition-transform duration-500 ${activeTab === "dashboard" ? "scale-110" : ""}`}
              />
              {!isSidebarCollapsed && (
            ...
            {[
              {
                ...
                label: "Properties",
                icon: Home,
                postTab: "house_post" as DashboardTab,
                viewTab: "house_view" as DashboardTab,
                color: "text-blue-500",
              },
              {
                id: "cars",
                label: "Vehicles",
                icon: Car,
                postTab: "car_post" as DashboardTab,
                viewTab: "car_view" as DashboardTab,
              ...
              },
              {
                ...
                label: "Services",
                icon: Wrench,
                postTab: "service_post" as DashboardTab,
                viewTab: "service_view" as DashboardTab,
              ...
              <div key={cat.id} className="space-y-1">
                <button
                  ...
                  className={`w-full flex items-center justify-between px-4 py-3.5 rounded-2xl hover:bg-muted/50 transition-all duration-300 text-muted-foreground hover:text-foreground group/item ${
                    activeTab === cat.postTab || activeTab === cat.viewTab
                ...
                >
                  <div className="flex items-center gap-3.5">
                    <cat.icon
                      className={`h-5 w-5 flex-shrink-0 transition-colors ${activeTab === cat.postTab || activeTab === cat.viewTab ? cat.color : "group-hover/item:text-primary"}`}
                    />
                    {!isSidebarCollapsed && (
                      <span className="font-bold text-sm tracking-tight">
                        {cat.label}
                  ...
                  {!isSidebarCollapsed && (
                    <ChevronDown
                      className={`h-3.5 w-3.5 transition-transform duration-500 ${expandedMenus.includes(cat.id) ? "rotate-180" : "-rotate-90 opacity-40"}`}
                    />
                  )}
                </button>
            ...
            <div className="space-y-12 max-w-7xl mx-auto animate-fade-in">
              <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                  ...
                  <p className="text-muted-foreground font-medium mt-1">
                    Real-time metrics for your DigitalBroker portfolio.
                  </p>
                </div>
                <div className="flex items-center gap-3">
                  <Button
                    variant="outline"
                    className="rounded-xl border-border/60 font-bold text-xs px-5 h-11 transition-all"
                    asChild
                  >
                    <Link href="/help">Export Report</Link>
                  </Button>
                  <Button
                    className="rounded-xl font-black text-xs px-6 h-11 shadow-lg shadow-primary/20 transition-all"
                    asChild
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {[
                  ...
                  <div
                    key={i}
                    className="group bg-card/50 backdrop-blur-sm border border-border/50 rounded-3xl p-8 shadow-sm hover:shadow-xl hover:shadow-primary/5 hover:border-primary/20 transition-all duration-500 relative overflow-hidden"
                  >
                    <div
                      className={`absolute top-0 right-0 h-32 w-32 ${stat.bg} blur-[60px] translate-x-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-700`}
                    />

                    <div className="flex items-start justify-between relative z-10">
                      <div
                        className={`p-4 ${stat.bg} rounded-2xl transition-transform duration-500 group-hover:scale-110`}
                      >
              ...
              <div className="bg-card/50 backdrop-blur-sm border border-border/50 rounded-[2.5rem] shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-700 delay-200">
                <div className="p-8 border-b border-border/40 flex items-center justify-between bg-card/30">
                  ...
                  <Button
                    variant="ghost"
                    className="text-primary font-black text-xs hover:bg-primary/5 px-4 rounded-xl"
                    onClick={() => setActiveTab("house_view")}
                  >
                  ...
                  <table className="w-full min-w-[800px]">
                    <thead>
                      <tr className="bg-muted/30 border-b border-border/40">
                        <th className="text-left py-5 px-8 text-[10px] font-black uppercase tracking-widest text-muted-foreground">
                          Listing Portfolio
                        </th>
                        <th className="text-left py-5 px-8 text-[10px] font-black uppercase tracking-widest text-muted-foreground">
                          Valuation
                    ...
                    <tbody className="divide-y divide-border/40">
                      {mockListings.map((listing) => (
                        ...
                        >
                          ...
                          <td className="py-6 px-8 text-muted-foreground text-xs font-semibold">
                            {listing.location}
                          </td>
                          <td className="py-6 px-8">
                            <div className="flex items-center gap-2">
                          ...
                          <td className="py-6 px-8 text-muted-foreground text-[11px] font-bold">
                            {listing.createdAt}
                          </td>
                        </tr>
                      ))}
            ...
            <div className="max-w-4xl mx-auto">
              <div className="mb-8">
                ...
                <p className="text-muted-foreground">
                  Fill in the details to list your property or service
                </p>
              </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (activeTab === "house_post")
                          setHouseForm((prev) => ({ ...prev, title: val }));
                        else if (activeTab === "car_post")
                          setCarForm((prev) => ({ ...prev, title: val }));
                        else
                          setServiceForm((prev) => ({ ...prev, title: val }));
                      }}
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        const val = e.target.value;
                        if (activeTab === "house_post")
                          setHouseForm((prev) => ({ ...prev, location: val }));
                        else if (activeTab === "car_post")
                          setCarForm((prev) => ({ ...prev, location: val }));
                        else
                          setServiceForm((prev) => ({
                            ...prev,
                            location: val,
                ...
                <div className="space-y-2">
                  ...
                  <textarea
                    ...
                    onChange={(e) => {
                      const val = e.target.value;
                      if (activeTab === "house_post")
                        setHouseForm((prev) => ({ ...prev, description: val }));
                      else if (activeTab === "car_post")
                        setCarForm((prev) => ({ ...prev, description: val }));
                      else
                        setServiceForm((prev) => ({
                          ...prev,
                          description: val,
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const val = e.target.value;
                      if (activeTab === "house_post")
                        setHouseForm((prev) => ({ ...prev, price: val }));
                      else if (activeTab === "car_post")
                        setCarForm((prev) => ({ ...prev, price: val }));
              ...
              <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <Button
                  onClick={() => {
                    ...
                    else if (activeTab === "service_view")
                      setActiveTab("service_post");
                  }}
                  className="gap-2"
                >
