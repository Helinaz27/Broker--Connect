...
import { User, Menu, X, Heart, LogOut, Settings, LayoutDashboard, ChevronDown, Bell, Home, Car, Wrench } from "lucide-react";
      ...
      <div className="container flex h-20 items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-1">
          ...
          <div className="relative">
            ...
            >
              Categories
              <ChevronDown className={`h-4 w-4 transition-transform duration-500 ${categoriesOpen ? "rotate-180" : ""}`} />
            </button>
            ...
            {categoriesOpen && (
              ...
              >
                {[
                  { href: "/house-listings", label: "Houses", desc: "Houses, Apartments & Land", icon: Home },
                  { href: "/car-listings", label: "Cars", desc: "Cars, Trucks & Rentals", icon: Car },
                  { href: "/service-listings", label: "Services", desc: "Experts & Professionals", icon: Wrench }
                ].map((item) => (
                  <Link
                    key={item.href}
                    href={item.href}
                  ...
                  >
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="text-[11px] text-muted-foreground leading-normal">{item.desc}</span>
                    </div>
                  </Link>
                ))}
              </div>
        ...
        <div className="hidden md:flex items-center gap-4">
          <div className="flex items-center gap-1">
            ...
            <Link href="/login">
              <Button variant="ghost" size="sm" className="text-muted-foreground hover:text-foreground hover:bg-muted/50 rounded-xl px-4 h-10 font-medium">
                Login
              </Button>
            </Link>
          </div>
          ...
          <HoverCard openDelay={0} closeDelay={200}>
            <HoverCardTrigger asChild>
              <Link href="/dashboard" className="flex items-center gap-3 p-1 pr-4 rounded-full bg-muted/30 hover:bg-muted/50 transition-all border border-transparent hover:border-border group">
                <Avatar className="h-8 w-8 border border-white dark:border-white/10 shadow-sm transition-transform group-hover:scale-105">
                  ...
                  <AvatarFallback className="bg-primary text-white text-[10px] font-bold">HZ</AvatarFallback>
                </Avatar>
            ...
            <HoverCardContent className="w-64 p-2 mt-2 glass-card rounded-2xl" align="end">
              <div className="flex flex-col gap-1">
                <div className="px-3 py-3 mb-1">
                  <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-1">Broker Profile</p>
                ...
                {[
                  ...
                  <Link key={item.href} href={item.href} className="flex items-center gap-3 px-3 py-2 rounded-xl hover:bg-primary hover:text-white transition-all text-sm font-medium text-muted-foreground hover:text-white group">
                    <item.icon className="h-4 w-4 opacity-70 group-hover:opacity-100 transition-opacity" />
                    <span>{item.label}</span>
                  </Link>
                ))}
                <div className="h-px bg-border/50 my-1 mx-2" />
                <Link href="/login" className="flex items-center gap-3 px-3 py-2 rounded-xl hover:bg-destructive/10 text-muted-foreground hover:text-destructive transition-all text-sm font-medium w-full text-left group">
      ...
      {mobileMenuOpen && (
        <div className="md:hidden border-t border-border/50 bg-background/95 backdrop-blur-xl py-8 animate-in slide-in-from-top-4 duration-300">
          <nav className="container flex flex-col gap-2">
            ...
            <p className="px-4 text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-2">Categories</p>
            {[
