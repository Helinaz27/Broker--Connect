...
export default function FilterSection({
  ...
  const inputStyles = "w-full px-4 py-3 bg-muted/50 border border-border/50 rounded-xl text-sm font-medium focus:outline-none focus:ring-4 focus:ring-primary/10 focus:border-primary transition-all placeholder:text-muted-foreground/50";
  const labelStyles = "text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-2 ml-1 block";
    ...
    return (
      <div className="flex flex-col lg:flex-row items-end gap-6 bg-card p-3 rounded-2xl shadow-soft">
        ...
        <div className="w-full lg:w-56">
          ...
          >
            {priceBrackets.map(p => <option key={p.value} value={p.value}>{p.label}</option>)}
          </select>
        </div>
  ...
  return (
    <div className="space-y-8 animate-in">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-bold text-foreground tracking-tight">Refine Portfolio</h3>
        {isFiltered && (
          <button onClick={clearFilters} className="text-[10px] font-bold uppercase tracking-widest text-primary hover:text-primary/80 transition-colors">
            Reset All
          </button>
        )}
      </div>
            ...
            {categories.map((cat) => (
              <button
                ...
                className={`px-4 py-3 rounded-xl text-[11px] font-bold uppercase tracking-widest border transition-all ${
                  filters.type === cat.value ? "bg-primary border-primary text-white shadow-lg shadow-primary/20" : "bg-muted/50 border-border/50 text-muted-foreground hover:border-primary/40 hover:text-foreground hover:bg-muted"
                }`}
              >
                {cat.label}
              </button>
            ...
            {priceBrackets.map((p) => (
              <button
                ...
                className={`px-4 py-3.5 rounded-xl text-[11px] font-semibold text-left border transition-all ${
                  `${filters.priceRange[0]}-${filters.priceRange[1]}` === p.value ? "bg-primary/5 border-primary text-primary" : "bg-muted/30 border-transparent text-muted-foreground hover:bg-muted/50 hover:border-border"
                }`}
              >
                {p.label}
              </button>
