...
export function Sidebar({
  ...
  return (
    <aside className={`${isSidebarCollapsed ? "w-20" : "w-72"} bg-card border-r border-border transition-all duration-300 flex flex-col z-40 relative shadow-sm`}>
      ...
      <nav className="flex-1 p-4 space-y-1.5 overflow-y-auto custom-scrollbar">
        ...
        >
          ...
          {!isSidebarCollapsed && <span className="font-semibold text-sm">Performance Hub</span>}
        </button>
          ...
          <div key={cat.id} className="space-y-1">
            ...
            >
              <div className="flex items-center gap-4">
                ...
                {!isSidebarCollapsed && <span className="font-semibold text-sm">{cat.label}</span>}
              </div>
              {!isSidebarCollapsed && (
