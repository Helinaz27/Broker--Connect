    ...
    <>
      ...
      >
        ...
        <nav className="flex-1 p-4 space-y-1.5 overflow-y-auto custom-scrollbar">
          ...
          >
            <LayoutDashboard
              className={`h-5 w-5 flex-shrink-0 transition-transform ${activeTab === "dashboard" ? "scale-105" : "group-hover:scale-105"}`}
            />
            {(!isSidebarCollapsed || isMobileSidebarOpen) && (
            ...
            <div key={cat.id} className="space-y-1">
              <button
                ...
                className={`w-full flex items-center justify-between px-4 py-3.5 rounded-xl transition-all group/item ${
                  activeTab === cat.post || activeTab === cat.view
              ...
              >
                <div className="flex items-center gap-4">
                  <cat.icon
                    className={`h-5 w-5 flex-shrink-0 transition-colors ${activeTab === cat.post || activeTab === cat.view ? cat.color : "group-hover/item:text-foreground"}`}
                  />
                  {(!isSidebarCollapsed || isMobileSidebarOpen) && (
                ...
                {(!isSidebarCollapsed || isMobileSidebarOpen) && (
                  <ChevronDown
                    className={`h-4 w-4 transition-transform duration-300 ${expandedMenus.includes(cat.id) ? "rotate-180" : "text-muted-foreground/50"}`}
                  />
                )}
              </button>
