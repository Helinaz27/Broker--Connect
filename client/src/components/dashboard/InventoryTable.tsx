      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-bold text-foreground tracking-tight italic">
            {activeTab === "house_view" ? "House Portfolio" : activeTab === "car_view" ? "Car Inventory" : "Service Directory"}
          </h1>
          <p className="text-muted-foreground font-medium mt-1">Review and manage your active {activeTab === "house_view" ? "houses" : activeTab === "car_view" ? "cars" : "services"}.</p>
        </div>
        <Button onClick={() => setActiveTab(activeTab.replace("view", "post") as any)} className="rounded-xl font-bold text-xs uppercase tracking-widest gap-2 bg-primary text-white">
          ...
          <thead className="bg-muted/50 border-b border-border">
            <tr>
              {["Asset Details", "Valuation", "Status", "Actions"].map(h => (
                <th key={h} className={`px-6 py-4 text-[10px] font-bold text-muted-foreground uppercase tracking-widest ${h === "Actions" ? "text-right" : ""}`}>{h}</th>
              ))}
            </tr>
          </thead>
                ...
                <td className="px-6 py-5">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">{listing.location}</p>
                    </div>
                  </div>
                </td>
                <td className="px-6 py-5 font-bold text-sm">{listing.price.toLocaleString()} Br</td>
                <td className="px-6 py-5">
                  <span className={`inline-flex items-center px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest ${listing.status === "active" ? "bg-emerald-500/10 text-emerald-600" : "bg-amber-500/10 text-amber-600"}`}>{listing.status}</span>
                </td>
