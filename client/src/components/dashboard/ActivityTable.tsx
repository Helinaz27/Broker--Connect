  ...
  return (
    <div className="bg-card border border-border rounded-3xl shadow-soft overflow-hidden">
      <div className="p-6 border-b border-border flex flex-col md:flex-row md:items-center justify-between gap-6">
        <h2 className="text-lg font-bold text-foreground tracking-tight">Activity Stream</h2>
          ...
          <thead>
            <tr className="bg-muted/30">
              {["Asset Details", "Valuation", "Type", "Status", "Actions"].map((h) => (
                <th key={h} className={`px-6 py-4 text-[10px] font-bold uppercase tracking-widest text-muted-foreground ${h === "Actions" ? "text-right" : ""}`}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody className="divide-y divide-border/50">
            {filteredListings.map((listing) => (
              <tr key={listing.id} className="group hover:bg-muted/20 transition-colors">
                ...
                <td className="px-6 py-5">
                  <span className={`px-3 py-1 rounded-full text-[9px] font-bold uppercase tracking-widest border ${listing.type === "rent" ? "bg-blue-500/10 text-blue-600 border-blue-500/20" : "bg-emerald-500/10 text-emerald-600 border-emerald-500/20"}`}>{listing.type}</span>
                </td>
                <td className="px-6 py-5">
                  <div className="flex items-center gap-2">
                    <div className={`h-1.5 w-1.5 rounded-full ${listing.status === "active" ? "bg-emerald-500" : "bg-amber-500"}`} />
                    <span className={`text-[10px] font-bold uppercase tracking-widest ${listing.status === "active" ? "text-emerald-600" : "text-amber-600"}`}>{listing.status}</span>
                  </div>
                </td>
