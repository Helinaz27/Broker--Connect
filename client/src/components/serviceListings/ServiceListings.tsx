// client/src/components/serviceListings/ServiceListings.tsx
"use client";
...
export default function ServiceListings({ queryParams }: ServiceListingsProps) {
  ...
  const toTableRow = (l: Listing) => ({
    ...
    // services have no listingMode, omit it
    status: l.status as any,
    createdAt: l.createdAt,
  });

  const handleView = (id: string) => router.push(`/service-listings/${id}`);
  ...
  if (isError) {
    return (
      <div className="flex flex-col items-center gap-3 py-16 text-sm text-muted-foreground">
        <p>Failed to load service listings.</p>
        <button
          onClick={refetch}
