...
const HOUSE_TYPES = ['all', 'apartment', 'villa', 'studio', 'duplex', 'penthouse'];
...
export default function HousesScreen() {
  ...
  const [applied, setApplied] = useState({ search: '', city: '', houseType: 'all', mode: 'all' });
  ...
  const applyFilters = () => { setApplied({ search, city, houseType, mode }); setPage(1); };
  const reset = () => { setSearch(''); setCity(''); setHouseType('all'); setMode('all'); setApplied({ search: '', city: '', houseType: 'all', mode: 'all' }); setPage(1); };
    ...
    <SafeAreaView style={s.safe}>
      ...
      <View style={s.filterBox}>
        <View style={s.inputRow}>
          ...
          <TextInput style={s.input} placeholder="Search houses..." placeholderTextColor={t.textMuted} value={search} onChangeText={setSearch} returnKeyType="search" onSubmitEditing={applyFilters} />
        </View>
        <View style={s.inputRow}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          {HOUSE_TYPES.map(h => (
            <TouchableOpacity key={h} style={[s.chip, houseType === h && s.chipActive]} onPress={() => setHouseType(h)}>
              <Text style={[s.chipText, houseType === h && s.chipTextActive]}>{h === 'all' ? 'All Types' : h.charAt(0).toUpperCase() + h.slice(1)}</Text>
            </TouchableOpacity>
          ))}
        </ScrollView>
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          {MODES.map(m => (
            <TouchableOpacity key={m} style={[s.chip, mode === m && s.chipActive]} onPress={() => setMode(m)}>
              <Text style={[s.chipText, mode === m && s.chipTextActive]}>{m === 'all' ? 'All Modes' : m.charAt(0).toUpperCase() + m.slice(1)}</Text>
            </TouchableOpacity>
          ))}
        </ScrollView>
      ...
      {isLoading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<View style={s.empty}><Ionicons name="business-outline" size={48} color={t.border} /><Text style={s.emptyText}>No houses found</Text></View>}
          renderItem={({ item }) => (
            <ListingCard
              id={item.id} title={item.title} price={item.price}
              location={item.location?.city ?? ''} image={item.images?.[0] ?? ''}
              category="house" listingMode={item.listingMode}
            />
          )}
          ListFooterComponent={
            totalPages > 1 ? (
...
function makeStyles(t: any) {
  return StyleSheet.create({
    ...
    inputRow: { flexDirection: 'row', alignItems: 'center', gap: 10, backgroundColor: t.inputBg, borderRadius: 10, paddingHorizontal: 12, borderWidth: 1, borderColor: t.border },
